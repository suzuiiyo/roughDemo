//
// Created by yanGw on 2021/3/8.
//

#ifndef CLIONCODE_INICPP2_H
#define CLIONCODE_INICPP2_H
...
namespace ini
{
    ...
    public:
        IniFile() : IniFile('=', '#')
        {}
        IniFile(const char fieldSep, const char comment) : fieldSep_(fieldSep), comentPrefixes_(1, std::string(1, comment))
        {}

        IniFile(const std::string &filename, const char fieldSep = '=', const char comment = '#') : IniFile(fieldSep, comment)
        {
                load(filename);
        }

        IniFile(std::istream &is, const char fieldSep, const std::vetor<std::string> &commentPrefixes)
        ...
        void decode(std::istream &is) {
            ...
            while (!is.eof() && !is.fail()) {
                ...
                if (line[0] == '[') {
                    ...
                    //check is section was already opened
